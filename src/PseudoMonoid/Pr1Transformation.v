...
Section ProdPsTrans.
  ...
  Proof.
    ...
    - exact (λ x, π₁).
    - exact (λ x y f, inv_of_invertible_2cell (pair_1cell_pr1 _ _ _)).
  Defined.

  Proposition pr1_pstrans_data_laws
    : is_pstrans pr1_pstrans_data.
  Proof.
    repeat split.
    ...
    - intros x y z f g ; cbn.
      ...
      use vcomp_move_L_Vp.
      rewrite <- rwhisker_vcomp.
      rewrite prod_psfunctor_comp_pr1.
      rewrite pair_2cell_pr1.
      etrans.
      {
  ...
  Definition pr1_pstrans
    : pstrans
        (comp_psfunctor (prod_psfunctor G₁ G₂) F)
        (comp_psfunctor G₁ F).
  Proof.
    use make_pstrans.
    - exact pr1_pstrans_data.
    - exact pr1_pstrans_data_laws.
  Defined.
End ProdPsTrans.
